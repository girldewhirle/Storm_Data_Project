...
```{r,echo=TRUE}
## created table of unique dates
unique_dates<-as.data.frame(table(storm_data$BGN_DATE))
```

```{r,echo=TRUE,fig.cap="Figure One - Frequency of reported severe weather events by date"}
qplot(Var1, Freq, data=unique_dates)
```
...
```{r,echo=TRUE}
storm_data<-filter(storm_data,(BGN_DATE>=ten_years_ago)&(BGN_DATE<=most_recent_date))
```
