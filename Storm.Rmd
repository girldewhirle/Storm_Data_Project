...
---
## Title: Evaluation of Population Health and Economic Damage Caused by Storms and Severe Weather in United States
...
Population Health was considered to include events that caused human fatalities or injuries.  Economic consequences was considered to include events that had caused damage to property or crops (estimates of actual dollar amounts).

## Data Processing

** Session Info**

To do: run sessionInfo() and include details in here.
...
Raw data was accessed from (include link)
...
Human impact records were subsetted on the basis that either they caused greater than zero fatalities OR they caused greater than zero injuries.  Records that did not meet these criteria were excluded as they have no bearing on answering the question about impact to population health/human impact.
...
Economic impact (damage) records were subsetted on the basis that either the PROPDMG (property damage) field was greater than zero OR that the CROPDMG (crop damage) field was greater than zero.  Records that did not meet these criteria were excluded as they have no bearing on answering the question about economic damage.
...
A variable of the 48 standard values from the [National Weather Service Storm Data Documentation] (https://d396qusza40orc.cloudfront.net/repdata%2Fpeer2_doc%2Fpd01016005curr.pdf) was created and converted to upper case.
...
The function below is to convert the exponential values in the PROPDMGEXP and CROPDMGEXP fields to a numerical value so the total dollar amount of damage can be calculated.  For the values in those variables, if they did not equal K, M or B (or k, m or b) the multiplier was returned as 1.
...
```{r,echo=TRUE}
kable(arrange(unique_top_human,EVTYPE),caption="Merge Table for Cleaning EVTYPE for Human Data")
```
...
## melt data
melted_economic_table<-melt(economic_table,id.vars=c("new_event","BGN_DATE","REFNUM"))
...
```
As is seen in the code above, for the two 'top 1%' subsets - human and economic - tables of unique event types and their frequencies were extracted.

The event type helper functions were run on these tables in order of event frequency until all listed events had a matching standardised value.  This was done on the human data and the economic data seperately.
...
Summary tables for human and economic impact were then created using the summarise function and are included in the results section.
...
Plot objects were created in ggplot2 and then combined into a single figure.

```{r,echo=TRUE}
## plot of total human impact by event type
human_plot_object<-ggplot(data=Mean_Human_Impact,aes(cleaned_EVTYPE,Total_Human_Impact))
## create object to plot as bar with text changed to read easier
hpo<-human_plot_object + geom_bar(stat="identity",colour="green",fill="blue") + theme(axis.text.x = element_text(angle = 90, hjust = 1))+xlab("Severe Weather Event Type")+ylab("Total Human Impact")+ggtitle("Total Human Damage by Weather Event Type")
```
...
## create object to plot as bar with text changed to read easier
epo<-economic_plot_object + geom_bar(stat="identity",colour="red",fill="yellow") + theme(axis.text.x = element_text(angle = 90, hjust = 1))+xlab("Severe Weather Event Type") + ylab("Sum of Total Economic Damage") + ggtitle("Total Economic Damage by Weather Event Type")
```

```{r,echo=TRUE,fig.cap="Figure One: Total Human and Economic Power by Weather Event Types"}
grid.arrange(hpo,epo,ncol=2)
```
